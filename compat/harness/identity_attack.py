...
from compat.harness.identity import PARTS, digest_of, identity
...
COVERED: Final[dict[str, tuple[str, ...]]] = {
    ...
    "gates": ("compat.just", "justfile", "pyproject.toml", "uv.lock", "conftest.py"),
}
...
def run_all() -> list[Control]:
    now = identity()
    return [*_coverage(now), *_sensitivity(now), _live_probe(), _uncovered_probe()]
