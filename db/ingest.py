...
def retract(conn, file_id: int, scope: str) -> None:
    ...
    no DELETE trigger, so a re-parse whose bytes had changed would strand a
    whole workflow graph in `blob` permanently.
    """
    owned = _OWNED[scope]
...
def _carrier(conn, file_id: int, carrier: str, slot: str, payload, now: float, parsed_by=None):
    ...
    # never sees the payload it displaces and a whole workflow graph stays in
    # `blob` per re-parse.
    conn.execute(
...
def one(conn, file_id: int, path, now: float, *, kind: str | None = None) -> Ingested:
    ...
    if kind in ("image", "animated_image", "video"):
        ...
        # The header this file already opened, when there is one. Skipped
        # outright only when the load above LOOKED AND FOUND NONE: a read that
        # threw is not a file without camera tags.
