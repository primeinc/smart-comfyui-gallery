...
REVIEW_QUALITY_EXPR = (
    "(SELECT rv.quality_score FROM ai_reviews rv WHERE rv.file_id = f.id ORDER BY rv.computed_at DESC LIMIT 1)"
)
REVIEW_ALIGNMENT_EXPR = (
    "(SELECT rv.prompt_alignment_score FROM ai_reviews rv WHERE rv.file_id = f.id ORDER BY rv.computed_at DESC LIMIT 1)"
)

# collections.name values for the built-in type='system_flag' collections.
STATUS_FLAG_VALUES: frozenset[str] = frozenset({"Approved", "Review", "To Edit", "Rejected", "Select"})
# Accepted values of the files.type column.
FILE_TYPE_VALUES: frozenset[str] = frozenset({"image", "video", "animated_image", "audio", "document"})
...
_SPECS: list[FieldSpec] = [
    ...
    _f(name="text", kind=Kind.TEXT, ops=_TEXT_CONTAINS, strategy=Strategy.ANY_TEXT, correlated=True),
    _f(
        name="name",
        kind=Kind.TEXT,
        ops=_TEXT_FULL,
    ...
    _f(name="path", kind=Kind.TEXT, ops=_TEXT_FULL, strategy=Strategy.COLUMN, column="f.path"),
    _f(name="folder", kind=Kind.TEXT, ops=frozenset({"eq", "contains"}), strategy=Strategy.FOLDER),
    _f(
        name="type",
        kind=Kind.ENUM,
        ops=_ENUM_CMP,
    ...
    _f(name="size_mb", kind=Kind.NUMBER, ops=_NUMBER_CMP, strategy=Strategy.EXPR, expr=SIZE_MB_EXPR),
    _f(
        name="mtime",
        kind=Kind.DATETIME,
        ops=_DATETIME_CMP,
    ...
    _f(
        ...
        expr=DURATION_SECONDS_EXPR,
        orderable=True,
        order_expr=DURATION_SECONDS_EXPR,
    ),
    ...
    _f(name="megapixels", kind=Kind.NUMBER, ops=_NUMBER_CMP, strategy=Strategy.EXPR, expr=MEGAPIXELS_EXPR),
    _f(name="is_favorite", kind=Kind.BOOL, ops=frozenset({"eq"}), strategy=Strategy.COLUMN, column="f.is_favorite"),
    _f(name="has_workflow", kind=Kind.BOOL, ops=frozenset({"eq"}), strategy=Strategy.COLUMN, column="f.has_workflow"),
    _f(
        name="workflow_prompt", kind=Kind.TEXT, ops=_TEXT_CONTAINS, strategy=Strategy.COLUMN, column="f.workflow_prompt"
    ),
    _f(name="workflow_files", kind=Kind.TEXT, ops=_TEXT_CONTAINS, strategy=Strategy.COLUMN, column="f.workflow_files"),
    _f(
        name="ai_caption",
        kind=Kind.TEXT,
        ops=frozenset({"contains", "is_null", "not_null"}),
        strategy=Strategy.COLUMN,
        column="f.ai_caption",
    ),
    _f(
        ...
        expr=RATING_AVG_EXPR,
        orderable=True,
        order_expr=RATING_AVG_EXPR,
        correlated=True,
    ),
    _f(
        name="rating_count",
    ...
    _f(
        ...
        kind=Kind.NUMBER,
        ops=_NUMBER_CMP,
        strategy=Strategy.MY_RATING,
        correlated=True,
        needs_client_uuid=True,
    ),
    _f(
        name="comment_count",
        kind=Kind.NUMBER,
    ...
    _f(
        ...
        kind=Kind.TEXT,
        ops=_TEXT_CONTAINS,
        strategy=Strategy.EXISTS_TEXT,
        table="file_comments",
        alias="c",
        inner_column="c.comment_text",
        correlated=True,
    ),
    _f(
        name="collection",
        kind=Kind.TEXT,
        ops=frozenset({"eq", "contains"}),
        strategy=Strategy.EXISTS_NAMED,
        type_filter="user_album",
        correlated=True,
    ),
    _f(
        name="status_flag",
        ...
        ops=_ENUM_CMP,
        strategy=Strategy.EXISTS_NAMED,
        type_filter="system_flag",
        enum_values=STATUS_FLAG_VALUES,
        correlated=True,
    ),
    _f(
        name="rated_by_user",
    ...
    _f(
        ...
        kind=Kind.NUMBER,
        ops=frozenset({"eq"}),
        strategy=Strategy.EXISTS_NUMERIC,
        requires_ai=True,
        correlated=True,
    ),
    ...
    _f(
        ...
        ops=_NUMBER_CMP,
        strategy=Strategy.SUBQUERY_SCALAR,
        expr=REVIEW_QUALITY_EXPR,
        orderable=True,
        order_expr=REVIEW_QUALITY_EXPR,
        requires_ai=True,
        correlated=True,
    ),
    _f(
    ...
    _f(
        ...
        kind=Kind.ENUM,
        ops=_ENUM_CMP,
        strategy=Strategy.EXISTS_ENUM,
        enum_values=REVIEW_ISSUE_VALUES,
        requires_ai=True,
        correlated=True,
    ),
    ...
    _f(
        ...
        ops=frozenset({"eq", "contains"}),
        strategy=Strategy.EXISTS_TEXT,
        table="generation_params",
        alias="gp",
        inner_column="gp.tool",
        correlated=True,
    ),
    _f(
        name="gen_model",
    ...
    _f(
        ...
        table="generation_params",
        alias="gp",
        inner_column="gp.loras",
        correlated=True,
    ),
    _f(
        name="gen_positive_prompt",
        kind=Kind.TEXT,
        ops=_TEXT_CONTAINS,
        strategy=Strategy.EXISTS_TEXT,
        table="generation_params",
        alias="gp",
        inner_column="gp.positive_prompt",
        correlated=True,
    ),
    _f(
        ...
        table="generation_params",
        alias="gp",
        inner_column="gp.negative_prompt",
        correlated=True,
    ),
    _f(
        name="gen_seed",
    ...
    _f(
        ...
        kind=Kind.FILE_REF,
        ops=frozenset({"eq"}),
        strategy=Strategy.FILE_REF,
        requires_ai=True,
        correlated=True,
    ),
    _f(
        name="similar_to_semantic",
        kind=Kind.FILE_REF,
        ops=frozenset({"eq"}),
        strategy=Strategy.FILE_REF,
        requires_ai=True,
