...
Every `Template` in this application states `media_type` rather than letting
Litestar work it out. Told nothing, `Template.to_response` guesses from the
template's own suffix (`guess_type(f"name{suffix}")`, litestar@2.24.0
response/template.py:135), and on Windows the first `guess_type` in a process
calls `mimetypes.init()`, which reads the whole MIME map out of the registry
(`_winapi._mimetypes_read_windows_registry`). That is paid once per process, so
ONE template left guessing would pay it for all of them. Stating it keeps
`mimetypes.inited` False and carries the same `text/html; charset=utf-8` either
way; every template this application has is `.html`, and one that is not must
state its own type.
"""
