...
def test_the_page_lays_out_the_verified_render_and_escapes_evidence(planned):
    ...
    # `/thumb` prefixes both addresses a member can have: `/thumbs/<shard>/<sha>.webp`
    # once its bytes are hashed, and the `/thumb/<slug>` route while they are not.
    # What this asserts is that every member is SHOWN, which is true of either.
    ...
    assert re.search(r'src="/static/build/app\.js\?v=\d+"', html), "the script carries the static version stamp"
