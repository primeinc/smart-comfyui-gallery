...
def test_the_v1_grammar_is_frozen_exact_and_exception_proof(monkeypatch):
    ...
    assert rendering.TemplateStoryRenderer().reads == {"snapshot": {1}, "plan": {2, 3, 4, 5, 6}}, (
    ...
    assert again["renderer"]["reads"] == {"snapshot": 1, "plan": 6}
    with pytest.raises(ValueError, match=r"reads StorySnapshot \[1\] and StoryPlan \[2, 3, 4, 5, 6\]"):
        rendering.TemplateStoryRenderer("memory").render(snapshot, {**plan, "v": 7}, snapshot_sha, plan_sha)
