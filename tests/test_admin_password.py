...
def test_a_restricted_mode_with_no_password_is_a_lockdown(smartgallery_app):
    ...
    _force_x, missing_x, _short_x = smartgallery_app.derive_login_policy(
        None, True, False)
...
def test_an_ordinary_local_install_is_neither(smartgallery_app):
    ...
    force_login, missing, too_short = smartgallery_app.derive_login_policy(
        None, False, False)
...
def test_a_short_password_is_flagged(smartgallery_app, password, expected):
    _force, _missing, too_short = smartgallery_app.derive_login_policy(
        password, False, False)

    assert too_short is expected, (
        f"{password!r} ({len(password)} chars) against a minimum of "
        f"{smartgallery_app.ADMIN_MIN_PASSWORD_LENGTH}")
...
@pytest.mark.parametrize("value", [
    123, ['a', 'b'], {'x': 1}, None, 'pässwörd-ünïcode', 'x' * 5000,
])
def test_a_crafted_password_cannot_crash_the_login_route(gallery_with_admin, value):
    ...
    resp = gallery_with_admin.app.test_client().post(
        '/galleryout/login', json={'username': 'admin', 'password': value})
