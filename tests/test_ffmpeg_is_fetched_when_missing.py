...
def test_the_build_is_gpl_because_the_stream_route_needs_x264(platform):
    ...
    assert "-gpl" in smartgallery.FFMPEG_BUILDS[platform]["asset"], (
        "the pinned asset is not a GPL build, so libx264 is missing and "
        "video playback cannot work")
...
def test_only_the_programs_come_out_and_they_come_out_flat(tmp_path):
    ...
    taken = smartgallery._extract_ffmpeg_programs(
        str(archive), str(out), build["programs"])
...
def test_a_tar_member_cannot_choose_where_it_lands(tmp_path):
    ...
    smartgallery._extract_ffmpeg_programs(str(archive), str(out),
                                          build["programs"])

    escaped = tmp_path.parent.parent / probe
    assert not escaped.exists(), f"a member escaped to {escaped}"
    assert (out / probe).read_bytes() == b"probe-bytes", (
        "the first matching member should win, not the last")
...
def test_an_archive_without_the_programs_is_an_error(tmp_path):
    ...
    with pytest.raises(OSError):
        smartgallery._extract_ffmpeg_programs(str(archive), str(tmp_path / "d"),
                                              build["programs"])
...
def offline(tmp_path, monkeypatch):
    ...
    _make_zip(archive, {f"ffmpeg-n8.1.2/bin/{probe}": b"probe-bytes",
                        f"ffmpeg-n8.1.2/bin/{ffmpeg}": b"ffmpeg-bytes"})
    ...
    monkeypatch.setattr(smartgallery, "ffmpeg_build_for_this_machine",
                        lambda: build)
...
def test_an_already_fetched_ffmpeg_is_used_without_downloading_again(
        offline, monkeypatch):
    ...
    monkeypatch.setattr(
        smartgallery, "_is_ffprobe",
        lambda path: os.path.normcase(str(path)).startswith(fetched_dir))
