...
def both_views(smartgallery_app):
    ...
    original = (
        smartgallery_app.concurrent.futures.ProcessPoolExecutor,
    ...
    names = [f"{_PREFIX}alpha_one.png", f"{_PREFIX}Beta_two.png", f"{_PREFIX}gamma_three.png"]
    ...
    try:
        ...
        coll_id = conn.execute("SELECT id FROM collections WHERE name = ?", ("Both Views",)).fetchone()[0]
        ...
        conn.execute("INSERT INTO file_ratings (file_id, client_uuid, rating) VALUES (?, 'admin', 5)", (ids[names[1]],))
        conn.execute(
...
def _shown(smartgallery_app, ids, url):
    ...
    return response.status_code, frozenset(name for name, file_id in ids.items() if file_id in body)
...
def test_a_filter_answers_the_same_in_both(smartgallery_app, both_views, query):
    ...
    folder_status, folder_files = _shown(smartgallery_app, ids, f"/galleryout/view/_root_?{query}")
    album_status, album_files = _shown(smartgallery_app, ids, f"/galleryout/collection/{coll_id}?{query}")

    assert folder_status == album_status == 200, (folder_status, album_status)
    assert folder_files == album_files, (
        f"{query} answers differently: the folder shows {sorted(folder_files)}, the album shows {sorted(album_files)}"
    )
...
def test_the_album_answers_the_same_in_json(smartgallery_app, both_views, query):
    ...
    page = client.get(url)
    as_page = frozenset(name for name, file_id in ids.items() if file_id in page.get_data(as_text=True))
    ...
    assert as_page == as_json, (
        f"{query}: the page shows {sorted(as_page)} and the visitor's list holds {sorted(as_json)}"
    )
...
def test_the_json_listing_is_really_being_read(smartgallery_app, both_views):
    ...
    payload = smartgallery_app.app.test_client().get(
        f"/galleryout/collection/{coll_id}", headers={"Accept": "application/json"}
    )
...
def test_the_filters_are_actually_filtering(smartgallery_app, both_views):
    ...
    answers = {
        query: _shown(smartgallery_app, ids, f"/galleryout/collection/{coll_id}?{query}")[1] for query in _FILTERS
    }
    ...
    assert answers["workflow_prompt=seed:5"] == {f"{_PREFIX}alpha_one.png"}, answers["workflow_prompt=seed:5"]
