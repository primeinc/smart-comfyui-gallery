...
def test_from_parsed_types_swarmui_stringified_values():
    ...
    parsed.params.update({
        ...
        "seed": "123456789", "steps": "20", "cfg": "7.0",
        "size": "1024x1024",
    })
...
def test_from_comfy_typed_graph_values():
    gp = GenerationParams.from_comfy({
        "seed": 42, "steps": 30, "cfg": 4.5, "sampler": "euler",
        "scheduler": "karras", "model": "flux1-dev.safetensors",
        "positive_prompt": "a red cube", "negative_prompt": "blurry",
...
def test_to_row_round_trips_through_sqlite_with_real_types():
    ...
    conn.execute(
        "CREATE TABLE generation_params (" + ", ".join(ROW_COLUMNS) + ")")
    gp = GenerationParams.from_comfy({
        "seed": 7, "steps": 25, "cfg": 6.5, "width": 512, "height": 768,
        "positive_prompt": "p", "negative_prompt": "n", "model": "m",
    })
    row = gp.to_row("file-1", 1000.0)
    assert len(row) == len(ROW_COLUMNS)
    conn.execute(
        "INSERT INTO generation_params VALUES ("
        + ", ".join("?" * len(ROW_COLUMNS)) + ")", row)
    got = conn.execute(
...
def test_omniquery_gen_fields_compile():
    ...
    cq = compile_query(vq, CompileParams(
        now_epoch=1000.0, base_path="/g", client_uuid=ctx.client_uuid,
        ai_resolutions={}))
