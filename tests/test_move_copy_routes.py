...
def _row(smartgallery_app, where_sql, params):
    ...
    try:
        row = conn.execute(
            f"SELECT id, path, name, is_favorite FROM files WHERE {where_sql}",
            params).fetchone()
...
def test_move_relocates_the_file_and_follows_it_in_the_database(
        smartgallery_app, client, dest_folder):
    ...
    resp = client.post("/galleryout/move_batch",
                       json={"file_ids": [file_id], "destination_folder": dest_key})
    ...
    assert os.path.normpath(moved["path"]) == os.path.normpath(
        os.path.join(dest_path, f"{_PREFIX}move_me.png"))
    assert moved["is_favorite"] == 1, "metadata was lost in the move"


def test_move_into_a_name_collision_never_overwrites(
        smartgallery_app, client, dest_folder):
    ...
    with open(incumbent, "rb") as fh:
        assert fh.read() == b"incumbent bytes that must survive", (
            "the move overwrote a file already at the destination")
    beside = [f for f in os.listdir(dest_path)
              if f.startswith(f"{_PREFIX}clash(") and f.endswith(".png")]
...
def test_move_into_the_same_folder_is_skipped(smartgallery_app, client):
    ...
    folders = smartgallery_app.get_dynamic_folder_config(force_refresh=True)
    root_key = next((k for k, v in folders.items()
                     if os.path.normpath(v["path"]) ==
                     os.path.normpath(smartgallery_app.BASE_OUTPUT_PATH)), None)
    if root_key is None:
        pytest.skip("gallery root is not exposed as a folder key")
    file_id, path = _seed(smartgallery_app, f"{_PREFIX}stay.png")

    resp = client.post("/galleryout/move_batch",
                       json={"file_ids": [file_id], "destination_folder": root_key})

    assert resp.status_code == 200
    assert os.path.exists(path), "a same-folder move removed the file"


def test_move_of_a_file_missing_from_disk_clears_its_row(
        smartgallery_app, client, dest_folder):
...
def test_copy_leaves_the_original_in_place(smartgallery_app, client, dest_folder):
    ...
    resp = client.post("/galleryout/copy_batch",
                       json={"file_ids": [file_id], "destination_folder": dest_key})
...
def test_move_rejects_an_unknown_destination(smartgallery_app, client):
    ...
    resp = client.post("/galleryout/move_batch",
                       json={"file_ids": [file_id], "destination_folder": "no_such_key"})
