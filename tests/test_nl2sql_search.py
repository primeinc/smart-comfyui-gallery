...
def db(tmp_path):
    ...
    conn.execute("CREATE TABLE files (id TEXT PRIMARY KEY, name TEXT, type TEXT, "
                 "workflow_prompt TEXT, is_favorite INTEGER)")
    ...
    conn.execute("CREATE TABLE generation_params (file_id TEXT PRIMARY KEY, "
                 "loras TEXT, positive_prompt TEXT)")
...
def test_sqlexec_rejects_non_select(db):
    for sql in ("DELETE FROM files", "UPDATE files SET is_favorite=1",
                "PRAGMA user_version", "ATTACH ':memory:' AS x",
...
def test_sqlexec_blocks_writes_at_engine_level(db):
    ...
    # data is untouched.
    r = run_readonly_select(db, "SELECT id FROM files WHERE id IN "
                                "(SELECT id FROM files); DELETE FROM files")
    assert not r.ok
