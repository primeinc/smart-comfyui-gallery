...
def test_every_floored_setting_is_read_through_env_num(gallery_tree, setting):
    ...
    assert call is not None, (
        f"{setting} is no longer assigned from env_num(...) at module "
        f"scope, so nothing applies a floor to it")
    floors = [kw for kw in call.keywords if kw.arg == "minimum"]
    assert floors, f"{setting} is read by env_num with no minimum="
    assert floors[0].value.value == _FLOORED[setting][1], (
        f"{setting}'s floor moved to {floors[0].value.value}")
...
def test_a_value_below_the_floor_falls_back(monkeypatch, capsys, name, bad):
    ...
    assert value == default, (
        f"{name}={bad} gave {value!r}, expected the default {default!r}")
...
def test_a_scan_still_runs_with_a_zeroed_batch_size(smartgallery_app, tmp_path,
                                                    monkeypatch):
    ...
    monkeypatch.setattr(smartgallery_app, "BATCH_SIZE",
                        smartgallery_app.env_num("BATCH_SIZE", 500, minimum=1))
    ...
    try:
        ...
        rows = conn.execute(
            "SELECT COUNT(*) FROM files WHERE name = ? AND REPLACE(path, '\\', '/') "
            "LIKE ?", ("floor.png", here + "%")).fetchone()[0]
        conn.execute("DELETE FROM files WHERE REPLACE(path, '\\', '/') LIKE ?",
                     (here + "%",))
        conn.commit()
    finally:
        conn.close()
