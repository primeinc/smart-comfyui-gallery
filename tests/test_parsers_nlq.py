...
def test_date_last_n_days():
    _exact("files from the last 30 days", {"field": "mtime", "op": "ge", "value": {"days_ago": 30}})


def test_date_last_n_weeks_scales():
    _exact("files from the last 2 weeks", {"field": "mtime", "op": "ge", "value": {"days_ago": 14}})


def test_date_last_bare_unit():
    _exact("files from the last week", {"field": "mtime", "op": "ge", "value": {"days_ago": 7}})


def test_date_between():
    _exact(
        "files between 2025-01-01 and 2025-06-01",
        {"field": "mtime", "op": "between", "value": ["2025-01-01", "2025-06-01"]},
    )


def test_date_month_year():
    _exact("files from March 2026", {"field": "mtime", "op": "between", "value": ["2026-03-01", "2026-03-31"]})
...
def test_collection_membership():
    _exact(
        "images in the Portfolio collection",
        {
            "op": "and",
            "children": [
...
def test_or_disjunction_both_sides():
    _exact(
        "favorite images or approved videos",
        {
            "op": "or",
            "children": [
                {
...
def test_coverage_guard_catches_dropped_media_type_disjunct():
    ast_dict = {"target": "files", "result": "ids", "where": {"field": "type", "op": "eq", "value": "image"}}
...
def test_coverage_guard_media_type_in_list_counts_as_present():
    ast_dict = {"target": "files", "result": "ids", "where": {"field": "type", "op": "in", "value": ["image", "video"]}}
    coverage, _missing = coverage_guard("images or videos", ast_dict)
    assert coverage == 1.0


def test_coverage_guard_catches_dropped_status_disjunct():
    ast_dict = {"target": "files", "result": "ids", "where": {"field": "status_flag", "op": "eq", "value": "Approved"}}
...
def test_text_field_compiles_and_matches_all_surfaces():
    ...
    conn.execute(
        "CREATE TABLE generation_params (file_id TEXT PRIMARY KEY, positive_prompt TEXT, model TEXT, loras TEXT)"
    )
    rows = [
    ...
    conn.execute(
        "INSERT INTO generation_params VALUES ('by_lora', '', '', '[{\"name\": \"girlnextdoor\", \"weight\": 0.8}]')"
    )
...
def test_extension_words_become_name_suffix(nl, suffixes):
    out = p.parse(nl, NOW)
    conds = [c for c in _iter_conds(out.ast["where"]) if c.get("field") == "name" and c.get("op") == "suffix"]
    assert sorted(c["value"] for c in conds) == suffixes, out.ast
    assert not any(
        c.get("field") == "text" and c.get("value") in {"pngs", "webp", "mp4s", "jpegs", "jpgs", "tiffs"}
...
def test_extension_or_group_renders_one_chip():
    out = p.parse("jpegs", NOW)
    assert [c["label"] for c in out.raw["interpretation"]] == ["name ends .jpg or name ends .jpeg"]
