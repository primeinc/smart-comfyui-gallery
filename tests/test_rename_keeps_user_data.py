...
def rated_file(smartgallery_app, monkeypatch):
    ...
    try:
        ...
        conn.execute("INSERT INTO collections (name, type, created_at) VALUES (?, 'user_album', 1.0)",
                     (f"{_PREFIX}album",))
...
def _attached(smartgallery_app, name):
    ...
    conn = smartgallery_app.get_db_connection()
    try:
        row = conn.execute("SELECT id, is_favorite FROM files WHERE name = ?",
                           (name,)).fetchone()
        if row is None:
            return None
        file_id = row[0]
        ...
        comments = conn.execute("SELECT COUNT(*) FROM file_comments WHERE file_id = ?",
                                (file_id,)).fetchone()[0]
        albums = conn.execute("SELECT COUNT(*) FROM collection_files WHERE file_id = ?",
                              (file_id,)).fetchone()[0]
...
def test_renaming_a_file_keeps_its_rating_and_comments(smartgallery_app, client, rated_file):
    resp = client.post(f"/galleryout/rename_file/{rated_file}",
...
def test_renaming_a_folder_keeps_the_ratings_of_the_files_inside(
    ...
    key = next(k for k, v in folders.items()
               if str(v["path"]).replace("\\", "/").endswith(f"{_PREFIX}box"))

    resp = client.post(f"/galleryout/rename_folder/{key}",
                       json={"new_name": f"{_PREFIX}renamedbox"})
...
def test_a_failed_rename_leaves_disk_and_database_agreeing(
    ...
    resp = client.post(f"/galleryout/rename_file/{rated_file}",
                       json={"new_name": f"{_PREFIX}nope.png"})
    monkeypatch.undo()
