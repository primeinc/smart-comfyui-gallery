...
def test_the_drift_check_can_actually_fail(tmp_path):
    ...
    from db.connect import connect, schema_sql
...
def test_the_build_control_counts_real_tables(db):
    ...
    assert len(real) == 62, f"expected 62 real tables, found {len(real)}: {sorted(real)}"
