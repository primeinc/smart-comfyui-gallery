...
def album(smartgallery_app, monkeypatch):
    ...
    names = [f"{_PREFIX}in_album.png", f"{_PREFIX}found_by_search.png",
             f"{_PREFIX}neither.png"]
    for name in names:
    ...
    try:
        ...
        conn.execute("INSERT INTO collections (name, type) VALUES (?, ?)",
                     ("Search Test Album", "user_album"))
        coll_id = conn.execute("SELECT id FROM collections WHERE name = ?",
                               ("Search Test Album",)).fetchone()[0]
        conn.execute("INSERT INTO collection_files (collection_id, file_id) "
                     "VALUES (?, ?)", (coll_id, ids[f"{_PREFIX}in_album.png"]))

        conn.execute("INSERT INTO omniquery_sessions (session_id, raw_sql) "
                     "VALUES (?, ?)", (session_id, "SELECT id FROM files"))
        conn.execute("INSERT INTO omniquery_results (session_id, file_id) "
                     "VALUES (?, ?)", (session_id, ids[f"{_PREFIX}found_by_search.png"]))
        conn.commit()
    finally:
        conn.close()
...
def test_the_album_shows_its_own_file(smartgallery_app, album):
    ...
    status, shown = _shown(smartgallery_app, ids,
                           f"/galleryout/view/collection_{coll_id}")
...
def test_a_search_from_inside_the_album_shows_its_results(smartgallery_app, album):
    ...
    status, shown = _shown(
        smartgallery_app, ids,
        f"/galleryout/view/collection_{coll_id}?omniquery_id={session_id}")

    assert status == 200
    assert f"{_PREFIX}found_by_search.png" in shown, (
        f"the search result is not on the page; shown: {shown}")
    assert f"{_PREFIX}in_album.png" not in shown, (
        f"the album's own contents came back instead of the search: {shown}")
...
def test_the_same_holds_for_the_ai_search(smartgallery_app, album):
    ...
    response = client.get(
        f"/galleryout/view/collection_{coll_id}?ai_session_id=no-such-session")
    ...
    assert "ai_session_id=no-such-session" in location, (
        f"the search was dropped on the way: {location}")
...
def test_going_to_the_album_without_a_search_is_unchanged(smartgallery_app, album):
    ...
    status, shown = _shown(
        smartgallery_app, ids,
        f"/galleryout/view/collection_{coll_id}?sort_by=name&sort_order=ASC")
...
def test_the_collection_view_still_ignores_these_parameters(gallery_tree, smartgallery_app):
    ...
    read = {call.args[0].value for call in ast.walk(view)
