...
def test_current_context_clustering_spans_subfolders_by_default(sg):
    ...
    html = client.get("/galleryout/view/_root_?cluster_mode=workflow&cluster_scope=current").get_data(as_text=True)
...
def test_collection_view_includes_subcollections_by_default(sg):
    ...
    with sg.get_db_connection() as conn:
        ...
        conn.execute(
            ...
            (parent_id, f_parent, _time.time()),
        )
        conn.execute(
            "INSERT INTO collection_files (collection_id, file_id, added_at) VALUES (?, ?, ?)",
            (child_id, f_child, _time.time()),
        )
        conn.commit()
    client = sg.app.test_client()
