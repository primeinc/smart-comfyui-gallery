...
def _upload(smartgallery_app, key, filename):
    ...
    return client.post("/galleryout/upload",
                       data={"folder_key": key,
                             "files": (io.BytesIO(buf.getvalue()), filename)},
...
def test_the_upload_still_refuses_a_disallowed_type(smartgallery_app, root_key):
    ...
    assert not os.path.exists(os.path.join(smartgallery_app.BASE_OUTPUT_PATH,
                                           f"{_PREFIX}payload.exe")), (
...
def test_the_sanitiser_removes_what_is_actually_dangerous(smartgallery_app,
                                                          hostile, expected):
