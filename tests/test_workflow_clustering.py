...
def test_schema_bump_marker_written_only_on_completion(sg, monkeypatch):
    ...
    with sg.get_db_connection() as conn:
        ...
        conn.execute(
            "INSERT OR REPLACE INTO ai_metadata (key, value, updated_at) VALUES ('cluster_hash_schema', 'stale', 0)"
        )
        conn.commit()
        calls = []
        monkeypatch.setattr(
        ...
        sg.check_and_update_workflow_hashes(conn)
        assert (
            conn.execute("SELECT value FROM ai_metadata WHERE key = 'cluster_hash_schema'").fetchone()[0]
            == sg.CLUSTER_HASH_SCHEMA
        )
...
def test_clustering_trigger_is_async_not_inline(sg, monkeypatch):
    ...
    kicked = []
    monkeypatch.setattr(sg, "ensure_cluster_backfill_async", lambda force_all=False: kicked.append(force_all) or True)
...
def test_backfill_selects_foreign_image_rows(sg, tmp_path, monkeypatch):
    ...
    with sg.get_db_connection() as conn:
        ...
        # Assert
        assert updated >= 1
        row = conn.execute("SELECT workflow_hash, prompt_hash FROM files WHERE id = ?", (_PREFIX + "fimg",)).fetchone()
...
def test_backfill_does_not_reselect_partially_hashed_rows(sg, tmp_path, monkeypatch):
    ...
    with sg.get_db_connection() as conn:
        _insert_file(
            conn, "pdone", path=str(target).replace("\\", "/"), has_workflow=0, workflow_hash="", prompt_hash="prOnly"
        )
        monkeypatch.setattr(sg, "compute_workflow_hashes", lambda p: calls.append(p) or ("", "", ""))
        ...
        row = conn.execute("SELECT prompt_hash, hash_failed FROM files WHERE id = ?", (_PREFIX + "pdone",)).fetchone()
...
def test_backfill_marks_missing_and_unparseable_files_failed_once(sg, tmp_path):
    ...
    with sg.get_db_connection() as conn:
        ...
        assert first == 0
        flags = dict(conn.execute("SELECT id, hash_failed FROM files WHERE id LIKE ?", (_PREFIX + "%",)).fetchall())
...
def test_backfill_force_all_retries_failed_rows(sg, tmp_path, monkeypatch):
    ...
    with sg.get_db_connection() as conn:
        ...
        assert (row["workflow_hash"], row["prompt_hash"], row["models_hash"], row["hash_failed"]) == (
            "wfNEW",
            "prNEW",
            "mdNEW",
            0,
...
def test_clear_synthetic_prompt_hashes_removes_only_synthetic_values(sg):
    ...
    with sg.get_db_connection() as conn:
        ...
        assert cleared == 1
        rows = dict(conn.execute("SELECT id, prompt_hash FROM files WHERE id LIKE ?", (_PREFIX + "%",)).fetchall())
